...
- ~~**FEL temporal builtins return Result<_, MissingTimezoneContextError> (PLN-0399)**~~ `fs-wbo9` · CLOSED — Migrate fel-core::current_date() and now() from infallible to Result<RFC3339Timestamp, MissingTimezoneContextError>.
...
Emit a warning per document — proposed `W713` (Theme, pass 6) and `W1704` (Experience, pass 9) —
...
- `W713` / `W1704` registered in the lint code table with spec refs to the two SHOULD lines.
