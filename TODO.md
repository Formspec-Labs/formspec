...
- ~~**Split formspec validate.py passes**~~ `fs-dtzx` · CLOSED — validate.py 1196 lines; duplicate *pass** loops.
...
- **P1 — Posture admission** (`for-n8c4`, `for-r8c4`, `for-s8c4` done): E608/E609 lint + shared matcher; formspec-server publish/submit bundle gate; studio branch-open gate (`Project.openAuthoringChangeset`, interim `ProposalManager` until ADR 0151 P3). Host wiring: [`../thoughts/deployment-posture-wiring.md`](../thoughts/deployment-posture-wiring.md). Fine-grained scope: [ADR 0152](../thoughts/adr/0152-multi-actor-authorization-scope.md) — per-actor half **landed** (accepted 2026-07-26, pass 1: `x-formspec-actor-scope` validated at branch-open, enforced at op-admission); per-class half **Held** pending pass 2 (§8).
