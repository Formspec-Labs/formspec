...
- Runtime plan execution for route/session behavior, per-Definition draft/submission state, idempotent host events, and product-neutral AI command events.
- Negative self-tests for stale sidecar refs, unadmitted contribution owners, unresolved nav targets, required-field runtime blocking, duplicate idempotency keys, and undeclared Screener hops.
...
| M5 | Runtime state semantics | Implemented initial pass | `src/runtime.ts`; `output/runtime-report.json` records two submitted Definition responses, route events, and zero runtime errors. |
...
| 6 | Route transitions fire durable/idempotent/undoable host events | Runtime emits host events with idempotency keys and rejects duplicate keys; undo remains unmodeled. |
...
4. **Runtime execution is viable at spike scope.** The runtime plan reaches `playbook-runner`, submits two Definition responses, emits two idempotent host events, and emits AI command issued/completed events with zero runtime errors.
