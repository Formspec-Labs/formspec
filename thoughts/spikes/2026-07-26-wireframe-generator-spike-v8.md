...
**Current disposition, 2026-07-29:** this section records the post-wall review
snapshot, not the current authoring interface. Wireframes-MCP now exposes
`declareDataSource`, `declareRegistry`, and `declareDefinition`; the current
Surface schema carries the corrected route-class vocabulary. The Surface v10
ledger and active tickets own remaining work.

- **Finding 2 is a facade gap, not a contract gap.** [`formspec/schemas/data-sources.schema.json`](../../schemas/data-sources.schema.json) already ratifies a `DataSource` shape with `kind`, `owner`, `scope`, `availability` (down to `level: "slot"` with `surfaceRef`/`routeRef`/`slotId`), `runtime` (delivery, cache, failure mode, provenance), and an optional payload `schema`; App Manifest carries `dataSources[]` siblings; ADR 0153 lists Data Sources as **Closed**. At this snapshot, Wireframes-MCP had no verb that authored it — zero occurrences of `dataSource` in the MCP or studio-core source — and validator/loader wiring remained tracked as `fs-r2od` (availability cross-artifact checks) and `fs-9d5e` (runtime loader). **This strengthened the promotion argument rather than weakening it:** the verb was a wrapper over a closed artifact, not a new contract.
...
- **Finding 6's proposed shape is rejected, and its "the graph cannot state it" is now false.** The tracker and the finding both propose `themeAuthority: tenant | platform | frozen` **on RoutePolicy**. Rejected on three independent grounds, and none of them is cost:
  ...
  **And the shipped vocabulary was then falsified.** [`../../../thoughts/experiments/2026-07-26-route-class-closure-test.md`](../../../thoughts/experiments/2026-07-26-route-class-closure-test.md) ran it over 35 product routes of this spike's own corpus: **28 truthful / 7 residual / 0 unassignable**, every residual in `operation`, which admitted tenant theming — so the residual bucket was **fail-open**, and a login page plus four MSA-referenced procurement pages were certified restylable. At this snapshot, the corrected vocabulary — `operation` refuses, plus new `attestation` and `authentication` values — lived only in [`../../../thoughts/adr/0159-product-substrate-recognition.md`](../../../thoughts/adr/0159-product-substrate-recognition.md) §The rendering ring. It has since propagated to the schema and specifications.
...
- **Everything else held at this snapshot.** The five-member slot-type union and the four-member `static-content` kind enum were as the persona found them, so findings 7-9, 12-14, 17, 22, 28, 31, 34, and 35 named real absences. The published MCP then had no Registry-admission verb and no `declareDefinition`, confirming 18 and 21 exactly.
