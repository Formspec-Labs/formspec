...
**Status:** in-progress — single named cross-artifact invariant landing this slice.
...
Validator + fixture + paired TS / Python conformance landed green. TS: `experience-action-refs-conformance.test.ts` passes 4/4 (positive resolution, dangling action id rejection, Response-Actions-not-loaded rejection, Experience-with-no-actionRefs positive). Python: `test_app_graph_experience_action_ref_fixture_corpus.py` passes 4/4 (corpus required-cases, source explicitness, diagnostic AppGraph ownership, no v4 spike key/path leakage). Broader app-graph vitest still 206/206 across 21 test files — zero regressions. The new `APP-GRAPH-EXPERIENCE-ACTION-REF` diagnostic carries through `runCrossArtifactValidators` and surfaces automatically through the existing `formspec-lint` and `formspec-server` publish-path consumers.
...
- Fixture: `formspec/tests/conformance/fixtures/app-graph-validator/experience-action-refs.case.json` (4 cases × handles).
- TS conformance runner: `formspec/packages/formspec-app-graph/tests/experience-action-refs-conformance.test.ts` (4/4 pass).
...
- Code review: dispatched after commit cluster lands.
