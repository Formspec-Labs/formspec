...
- **2026-05-23 r2.2** — execution-prep correction. The r2 `Generation.anchors` $def used `type: object` with `propertyNames.pattern`, but the existing `x-generation.anchors` at `component.schema.json:272-279` is `type: array` with `items.pattern`. Corrected to array-of-pattern-strings to match. This is a structural-shape error caught at Task 1 read-time, not a normative drift.
- **2026-05-23 r2.1** — applied Round-3 remediations. (i) Task 6 Step 4 adds explicit structural pin clarifying `authoredBy` is a root-level property parallel to existing `changes`/`attachmentBinding`/`actionEvent`; ADR §5.4's "payload field" is informal — there's no payload sub-object given `additionalProperties: false` at root (Pass A R3 HIGH). (ii) Task 7 Step 6 corrected path to `raw-project.ts:441-467` (NOT `project.ts` — that's the command handler); added `formspec-types` package to the type-source migration; explicitly scoped studio-core's `Project.export()` wrapper as out of P0 per existing Deviations §Cross-stack coordination + studio-cascade deferral (Pass B R3 HIGH). (iii) Task 12 file list trimmed — Pass B R3 grep-verified that 5 of r2's enumerated consumers (`sync-lint-schemas.mjs`, `copy-layout-css-assets.mjs`, `packages/formspec-layout/package.json`, `platform-defaults.ts`, `default-theme.json`) consume the runtime `.json` document (which doesn't retire), NOT the `.schema.json`. Only `schema_validation.rs:44` actually consumes the schema. Kept on the modify list: schema delete, mirror delete, `generate-theme-from-registry.mjs`, `pass_theme/token_registry.rs`, `schema_validation.rs`.
...
"Generation": {
  ...
  "properties": {
    ...
    "anchors":      {
      "type": "array",
      "description": "Existing field: source anchors with standard prefix and source-layer-owned suffix (see ADR §5.3 anchor-uniqueness invariant). Preserved as array-of-pattern-strings to match existing x-generation shape at component.schema.json:272-279.",
      "items": {
        "type": "string",
...
- [ ] **Step 4: Implement.**
  - respondent-ledger-event: add envelope-level `authoredBy: { $ref: 'common.schema.json#/$defs/AuthorActor' }`. **Critical structural pin:** the schema is FLAT — `additionalProperties: false` at the root with a single `properties` block (`respondent-ledger-event.schema.json:19`). The new `authoredBy` lands as a **root-level property parallel to `changes`, `attachmentBinding`, `actionEvent`, `data`**, etc. The ADR §5.4 prose phrase "payload field" is informal — it means "a top-level field on the event document, distinct from the envelope's existing `actor`," not a field inside a nested payload sub-object. There is no payload sub-object; do not invent one. ADR §5.4's "no new envelope shape" confirms: no new wrapper object, just one more sibling property at the root. Add an `if/then` block at root: `if: { properties: { eventType: { pattern: '^(ai\\.|user\\.)' } } }, then: { required: ['authoredBy'] }`. Spec prose distinguishes the two identities: envelope `actor` answers "in what respondent context did this event happen?" (envelope-level Actor, respondent-identity); envelope `authoredBy` answers "who issued the authoring op?" (AuthorActor, authoring-identity).
...
- Per Pass 2 codebase grounding: **no code-level consumer of `bundle-manifest.schema.json` exists today.** The "refactor every consumer" framing in r0 was misleading. Real consumer surface:
  ...
  - **Parallel `project.export()` shape** at `formspec/packages/formspec-core/src/raw-project.ts:441-467` (NOT `project.ts` — that's the command handler) returns `ProjectBundle` with singular `definition`. The `ProjectBundle` type re-exports from `@formspec-org/types` via `packages/formspec-core/src/types.ts:359-360`, so the type-source migration also touches `formspec/packages/formspec-types/`. **22+ call sites across 7 files** in `packages/formspec-core/tests/` consume the singular `.definition` accessor (Pass 3 R3 spot-check). The `formspec-studio` cascade is even broader (`formspec-studio/packages/formspec-studio-core/src/project.ts:190` wraps `core.export()` as `ProjectBundle`; `project-preview.ts:77` reads `project.core.export().definition`; multiple studio test files mirror). This is a TypeScript export shape mirroring the OLD bundle-manifest design without schema validation — Pass 2 R2 flagged it as silent-divergence risk per formspec `CLAUDE.md` §Development Philosophy. Task 7 closes the silent-divergence at the **formspec-core layer** (raw-project.ts + formspec-types + in-package tests); studio-core cascade is post-P0 per Deviations §Cross-stack coordination + the §formspec-studio cascade bullet. Plan-scope-merit decision: extending Task 7 to studio-core would balloon the BREAKING into a 50+-file change spanning two submodules; deferring keeps Task 7 atomic to the formspec submodule, matches the existing studio-cascade deferral pattern, and the silent-divergence at the studio-core wrapper is documented as a known post-P0 cleanup.
...
- [ ] **Step 6: Align `ProjectBundle` shape** at the formspec-core boundary.
  - **`formspec/packages/formspec-types/`** — locate the `ProjectBundle` (or equivalent) type definition; change `definition: DefinitionEnvelope` to `definitions: DefinitionEnvelope[]`. The type is the source-of-truth for the export shape.
  - **`formspec/packages/formspec-core/src/raw-project.ts:441-467`** — refactor the `export(): ProjectBundle` body. Today's bundle construction populates `definition: this._state.definition`; refactor to `definitions: [this._state.definition]` (single-element array; multi-definition support is P1+, but the schema shape carries the singular case as an array of length 1).
  - **In-package call sites** — grep `\\.export\\(\\)\\.definition\\|bundle\\.definition\\b` under `packages/formspec-core/tests/` and update each call to read `[0]` from the array. Pass B R3 spotted 22+ call sites across 7 files; enumerate at execution time.
  - **`formspec/packages/formspec-core/src/types.ts:359-360`** — if `ProjectBundle` is locally re-exported, follow the type-source change through.
  - **Studio-core cascade is OUT OF SCOPE.** `formspec-studio/packages/formspec-studio-core/src/project.ts:190` (wraps `core.export()`) and `project-preview.ts:77` (reads `.definition`) continue to consume the singular shape until the post-P0 studio cascade migrates them. This is a known silent-divergence at the studio-core wrapper layer and is documented in Deviations §Cross-stack coordination. The substrate-side fix is complete; the studio-side fix lands with the formspec-studio submodule-pointer bump.
...
**Files (r2.1 trimmed — Pass B R3 grep-verified that 5 r2 candidates consume the runtime `.json` document, NOT the `.schema.json`; only `schema_validation.rs:44` actually consumes the schema):**
...
- Modify: `formspec/crates/formspec-lint/src/schema_validation.rs:44` — `const TOKEN_REGISTRY_SCHEMA: &str = include_str!("../schemas/token-registry.schema.json");` references the retired schema. Drop the include + the constant + any downstream usage; re-route validation to the unified Registry's `token-category` contribution shape (the same migration as `pass_theme/token_registry.rs`).
- Final sweep: `grep -rln 'token-registry\.schema\.json' formspec/ /Users/mikewolfd/Work/formspec-stack/.claude-plugin/` — confirms zero remaining stack-internal references (vendored copies in sibling submodules excluded).
- **NOT MODIFIED** (these consume the runtime `.json` document, which doesn't retire — Pass B R3 grep-verified):
  - `formspec/scripts/sync-lint-schemas.mjs:17` (filters `token-registry.json`)
  - `formspec/scripts/copy-layout-css-assets.mjs:18` (lists `token-registry.json`)
  - `formspec/packages/formspec-layout/package.json:23` (exports `./token-registry: ./dist/token-registry.json`)
  - `formspec/packages/formspec-layout/src/platform-defaults.ts:4` (`import tokenRegistry from './token-registry.json'`)
  - `formspec/packages/formspec-layout/src/default-theme.json:2` (regen marker citing `token-registry.json`)
- Preserve `formspec/schemas/token-registry.json` AS-IS — production-runtime canonical document.
