...
async function main() {
  ...
  console.log("[spike-v4] validating source artifacts...");
  const ajv = await buildValidator();
  let validationFailures = 0;
  validationFailures += await validateOne(ajv, "https://formspec.org/spikes/wireframe-generator-v4/app-manifest/0.1", "App Manifest", inputs.appManifest);
  validationFailures += await validateOne(ajv, "https://formspec.org/schemas/registry/v1.0/registry.json", "Registry", inputs.registry);
  validationFailures += await validateOne(ajv, "https://formspec.org/spikes/wireframe-generator-v4/surface/0.1", "Surface", inputs.surface);
  ...
  validationFailures += await validateOne(ajv, "https://formspec.org/spikes/wireframe-generator-v4/runtime-plan/0.1", "Runtime Plan", runtimePlan);

  console.log("[spike-v4] validating whole app graph coherence...");
  ...
  console.log(`[spike-v4]   app coherence errors: ${coherence.summary.errors}`);
