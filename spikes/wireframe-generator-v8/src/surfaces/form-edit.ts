...
export const formEdit: SurfaceScript = {
  ...
  async author({ mcp, gap, bindRoute }) {
    // ── FINDING 18 — no admission path for a first-party product module
    gap({
      id: 18,
      verb: 'declareModule / bindSlot(module-widget)',
      ...
      got:
        '`declareModule` accepts the id, then the validator emits MODULE-UNRESOLVED and MODULE-CONTRIBUTION-MISSING because the Registry has no entry. The MCP surface has no verb to admit a module or to point the validator at a registry.',
      severity: 'reshape-needed',
      why:
        'The product\'s own designer cannot be composed into the product\'s own app graph through the product\'s own authoring MCP. Every module-bearing surface in this corpus fails the same way, which is why MODULE-* is the highest-count diagnostic in the run.',
      v7Ref: null,
      suggestion:
        'A `declareRegistry` / `useRegistry` verb on the MCP so authoring journeys can resolve against a real module index, plus a first-party registry entry for the studio canvas.',
    });
        ...
        slots: [
          ...
          },
          {
            ...
            binding: {
              ...
              config: {
                dataSource: 'x-spike-v8:workspace:forms/:id/draft-definition',
                groups: ['fields', 'binds', 'rules', 'locales', 'theme', 'signature'],
                publishes: 'x-spike-v8:selection:bind',
              },
            },
            title: 'Spec',
            position: 'left',
          ...
          },
          {
            ...
            binding: {
              ...
              config: {
                consumes: 'x-spike-v8:selection:bind',
          ...
          },
          {
            ...
            binding: {
              ...
              config: {
                dataSource: 'x-spike-v8:workspace:forms/:id/lint',
