/** @filedesc Source conformance for Surface definition-form slot binding cross-artifact resolution. */
...
const FIXTURE_PATH = resolve(
  fileURLToPath(new URL('../../../tests/conformance/fixtures/app-graph-validator/surface-definition-slots.case.json', import.meta.url)),
);
...
ajv.addSchema(readJson(resolve(SCHEMAS_ROOT, 'issuer.schema.json')));
ajv.addSchema(readJson(resolve(SCHEMAS_ROOT, 'definition.schema.json')));

const schemaValidators = {
  appManifest: ajv.compile(readJson(resolve(SCHEMAS_ROOT, 'bundle-manifest.schema.json'))),
  surface: ajv.compile(readJson(resolve(SCHEMAS_ROOT, 'surface.schema.json'))),
  definition: ajv.compile({ $ref: 'https://formspec.org/schemas/definition/1.0' }),
};
...
describe('Surface definition-form slot cross-artifact source conformance fixtures', () => {
