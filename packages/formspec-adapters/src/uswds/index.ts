...
// Generated by scripts/build-css.mjs, sibling to uswds-integration.css (ADR 0064 decision 2) — never
// hand-listed. A variant built from the same partial ships its own pair next to its own compiled sheet.
import { classVocabulary } from '../../uswds-integration.classes.js';
 ...
 * Hosts import nothing. Every input but DatePicker uses native behavior or `bind()` alone — no USWDS
 * component JavaScript. DatePicker (`date` fields) dynamically imports USWDS's own `usa-date-picker` module
 * when `@uswds/uswds` is installed (optional peer) and falls back to a plain text input otherwise.
 */
