...
// ── W712: Theme widget/dataType compatibility ─────────────────
...
fn w712_theme_select_multichoice_requires_multiple() {
    ...
    let def = json!({
        "$formspec": "1.0",
        "items": [{ "key": "tags", "type": "field", "dataType": "multiChoice" }]
    });
    let diags = lint_theme(&theme, Some(&def));
    let w712 = with_code(&diags, "W712");
    assert_eq!(w712.len(), 1);
    assert!(w712[0].message.contains("widgetConfig.multiple"));
}
...
fn select_multichoice_multiple_true_no_w712() {
    ...
    let diags = lint_theme(&theme, Some(&def));
    assert!(with_code(&diags, "W712").is_empty());
}

#[test]
fn textinput_uri_selector_no_w712() {
    let theme = json!({
        "selectors": [{
            "match": { "dataType": "uri" },
...
#[test]
fn moneyinput_integer_selector_no_w712() {
    let theme = json!({
        "selectors": [{
...
#[test]
fn w712_selector_select_multichoice_requires_multiple() {
    let theme = json!({
        "selectors": [{
            "match": { "dataType": "multiChoice" },
            "apply": { "widget": "Select" }
        }]
    });
    let diags = lint_theme(&theme, None);
    let w712 = with_code(&diags, "W712");
    assert_eq!(w712.len(), 1);
    assert!(w712[0].path.contains("selectors[0].apply.widget"));
}

#[test]
fn selector_select_multichoice_multiple_true_no_w712() {
    let theme = json!({
        "selectors": [{
            "match": { "dataType": "multiChoice" },
            "apply": {
...
fn w712_theme_widget_incompatible_with_definition_data_type() {
    ...
    let w712 = with_code(&diags, "W712");
    assert_eq!(w712.len(), 1);
    assert!(w712[0].message.contains("Toggle"));
    assert!(w712[0].message.contains("integer"));
}
