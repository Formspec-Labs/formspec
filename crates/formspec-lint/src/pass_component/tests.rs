...
#[test]
fn select_multiple_is_compatible_with_multichoice() {
    let comp = json!({
        "tree": {
            "component": "Stack",
            "children": [
                { "component": "Select", "bind": "tags", "multiple": true }
            ]
        }
    });
    let def = json!({
...
#[test]
fn select_without_multiple_is_incompatible_with_multichoice() {
    let comp = json!({
        "tree": {
            "component": "Stack",
            "children": [
                { "component": "Select", "bind": "tags" }
            ]
        }
    });
    let def = json!({
    ...
    let e802 = with_code(&diags, "E802");
    assert_eq!(e802.len(), 1);
    assert!(e802[0].message.contains("Select"));
    assert!(e802[0].message.contains("multiChoice"));
}
