...
| 6.2 | **Canonical JSON (JCS-lite)** | UTF-8, deterministic key sort (recursive), no whitespace, `authoredSignatures` omitted before hashing. Domain separation: `formspec.response.signed-payload.v1`. | Adding co-signatures does not invalidate earlier signatures. Cross-system verification. | ✅ | ✅ | — | — |
