...
|---|---|---|---|---|
| `#/$defs/AuthoredSignature/properties/signatureMethod` | yes | string | Registry-bound signature method URI that selects the cryptographic adapter and algorithm. | URI identifying the cryptographic signature method from the Formspec signature-method registry. Selects the adapter and algorithm for primitive verification. |
| `#/$defs/AuthoredSignature/properties/signatureValue` | yes | string | COSE_Sign1 signature bytes verified by the selected signature adapter. | Base64-encoded COSE_Sign1 byte string (RFC 9052). Detached payload; canonical signed bytes live in signedPayload.digest. The signatureMethod URI selects the cryptographic suite from the Formspec signature-method registry. |
