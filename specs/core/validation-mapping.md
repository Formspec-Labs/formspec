...
4. **One table, one truth.** §6 is the master table. Per ADR 0150 §4.2/§10, the canonical 5 rows are pinned at fixture authority (`tests/conformance/fixtures/validation-mapping/closed-core-5-rows-jcs.json`, JCS RFC 8785 byte-equality) — the schema's `MasterTable` carries only the row-shape contract. Pytest pins both layers (`tests/conformance/spec/test_validation_mapping_table.py` for fixture↔§6 prose; `tests/conformance/test_validation_mapping_master_table_demotion.py` for the four-constraint demotion + JCS byte equality).
...
This section is the **load-bearing reconciliation** required by concept §9 row 3 ("one table that reconciles action intent, Core global modes, per-shape timing, `ValidationSummary.source`, severity, and Response status transitions"). The table below is normative. Per ADR 0150 §4.2/§10, byte-equality authority for the canonical 5 rows lives at the fixture (`tests/conformance/fixtures/validation-mapping/closed-core-5-rows-jcs.json`, JCS RFC 8785) — the schema's `MasterTable` is open-cardinality and carries only the row-shape contract. The pytest in `tests/conformance/spec/test_validation_mapping_table.py` MUST pin the fixture↔prose match; the pytest in `tests/conformance/test_validation_mapping_master_table_demotion.py` MUST pin the four-constraint demotion + JCS byte equality.
