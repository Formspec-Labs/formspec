...
- AppGraphValidator emits UI Graph Policy diagnostics for target Surface mismatch, route coverage, duplicate route policies, unresolved routes, responsive slot references, hidden Definition refs that are unresolved or not route-local, loaded Locale $module.* keys without declared owners, overlapping Locale owner prefixes across different modules, and Locale owner keyPrefix/moduleId segment mismatch.
...
- This slice proves schema shape, generated types, structural fixture acceptance, host-evidence boundary, host-evidence schema result reporting, Surface/route semantic diagnostics, policy-local Locale-owner executable diagnostics, hidden Definition reference diagnostics, semantic fixture evidence, and report-origin readiness only; ModuleResolver-backed module-id resolution, Theme token-slot checks, ModuleResolver/Registry token-slot checks, Studio feedback, runtime hidden-state behavior, and any future App Manifest policy slot remain later gates.
