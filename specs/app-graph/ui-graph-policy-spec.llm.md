...
- AppGraphValidator emits UI Graph Policy diagnostics for target Surface mismatch, route coverage, duplicate route policies, unresolved routes, responsive slot references, loaded Locale $module.* keys without declared owners, and overlapping Locale owner prefixes across different modules.
...
- This slice proves schema shape, generated types, structural fixture acceptance, host-evidence boundary, host-evidence schema result reporting, Surface/route semantic diagnostics, Locale-owner executable diagnostics, semantic fixture evidence, and report-origin readiness only; Theme token-slot checks, hidden Definition behavior, ModuleResolver/Registry token-slot checks, Studio feedback, runtime hidden-state behavior, and any future App Manifest policy slot remain later gates.
