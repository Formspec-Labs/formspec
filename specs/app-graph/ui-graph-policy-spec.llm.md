...
- AppGraphValidator emits UI Graph Policy diagnostics for target Surface mismatch, route coverage, duplicate route policies, unresolved routes, responsive slot references, hidden Definition refs that are unresolved or not route-local, loaded Locale $module.* keys without declared owners, overlapping Locale owner prefixes across different modules, Locale owner keyPrefix/moduleId segment mismatch, and Locale owner moduleId references that are missing or not admitted in completed ModuleResolver evidence.
