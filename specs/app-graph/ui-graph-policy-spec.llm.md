...
- This slice proves schema shape, generated types, structural fixture acceptance, host-evidence boundary, host-evidence schema result reporting, Surface/route semantic diagnostics, policy-local and ModuleResolver-backed Locale-owner executable diagnostics, hidden Definition reference diagnostics, Theme widgetRef ModuleResolver evidence diagnostics, executable Theme token-slot checks over ModuleResolver token-slot evidence, semantic fixture evidence, and report-origin readiness only; token-category compatibility, Studio feedback, runtime hidden-state behavior, and any future App Manifest policy slot remain later gates.
