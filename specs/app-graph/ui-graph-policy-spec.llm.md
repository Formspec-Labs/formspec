...
- ModuleResolver may carry Registry widgetShape.tokenSlots[] evidence as widgetTokenSlots[] on resolved widget contributions, but AppGraphValidator does not yet emit THEME-TOKEN-SLOT.
...
- This slice proves schema shape, generated types, structural fixture acceptance, host-evidence boundary, host-evidence schema result reporting, Surface/route semantic diagnostics, policy-local and ModuleResolver-backed Locale-owner executable diagnostics, hidden Definition reference diagnostics, Theme widgetRef ModuleResolver evidence diagnostics, ModuleResolver token-slot evidence, semantic fixture evidence, and report-origin readiness only; executable Theme token-slot checks, token-category compatibility, Studio feedback, runtime hidden-state behavior, and any future App Manifest policy slot remain later gates.
