...
- Definition-response source schemas describe `Response.data`; processors resolve `definitionRef` exactly, reject direct top-level schema properties that are not Definition root data keys, and fail closed for unsupported top-level schema compositions.
