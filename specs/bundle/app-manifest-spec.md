...
# Formspec App Manifest Specification v2.4
...
- **App Manifest** -- A JSON document conforming to this specification, identified by `$formspecBundle: "2.0"`, `"2.1"`, `"2.2"`, `"2.3"`, or `"2.4"`. The author-facing composition document above a Formspec app. (The wire constant retains the `$formspecBundle` name for symmetry with the v1.0 lineage and to keep validator dispatch trivial; the **document** is the App Manifest.)
...
The Formspec architecture composes a form from up to ten authored siblings --
Definition, Experience, Response Actions, Component, Theme, Locale, Mapping,
References, Ontology, Registry -- plus generated and response-scoped artifacts.
Each sibling carries its own URL and version. Most form-bound siblings carry
`targetDefinition`; Component 1.2 may carry `targetSurfaceRoutes[]`, and Locale
2.0 carries `target`.
...
The App Manifest spec itself is versioned via `$formspecBundle`. This schema accepts `"2.0"`, `"2.1"`, `"2.2"`, `"2.3"`, and `"2.4"`:
...
A processor MUST check `$formspecBundle` before structural schema validation
and reject a newer unsupported value with `unsupported-bundle-version`. A v2.4
processor MUST accept valid older 2.x documents under their historical member
and entry rules. Those older rules do not create aliases on the 2.4 path:
`entrySurface` is valid only in 2.4, and an App Manifest 2.4 production bundle
must load Locale 2.0 rather than translate `targetDefinition`. Documents
carrying the legacy `"1.0"` value MUST be rejected by 2.x processors; the
1.0 → 2.0 plural-shape change is breaking and silent acceptance would misread
the document.
...
For form-bound siblings, a manifest's sibling pin SHOULD be a subset of the
sibling artifact's `targetDefinition.compatibleVersions` range or Locale 2.0
`target.compatibleVersions` range. Manifests MUST NOT widen a sibling's
compatibility range.
...
| `screeners` | `SiblingRef` | (URL) | Screener documents. v2.3+; multi-element supports explicit Screener-to-app association without absorbing Screener evaluation, runtime routing, TraceIndex, or hostEvidence into App Manifest. |
...
`entrySurface` is a canonical absolute URL and is valid only when
`$formspecBundle` is `"2.4"`. It selects one manifest Surface reference:

- With zero `surfaces[]` entries, `entrySurface` MUST be absent. The app has no
  renderable Surface entry.
- With one `surfaces[]` entry, `entrySurface` MAY be absent. When absent, the
  sole Surface is selected.
- With two or more `surfaces[]` entries, `entrySurface` is REQUIRED.
- When present, it MUST exactly match one `surfaces[].url`.

Two or more Surfaces without `entrySurface` fail with
`APP-ENTRY-AMBIGUOUS`. An explicit selector that does not match exactly one
manifested and loaded Surface fails with `APP-ENTRY-SURFACE-UNRESOLVED`.
Both failures produce no selected Surface and no fallback. A processor MUST NOT
choose by array order, local `Surface.id`, filename, route id, or a normalized
URL.

The selected Surface remains authoritative for the route. Its own `entry`
...
- A manifest without `locales` MUST cause Definition text to fall back to
...
Most form-bound sibling artifacts carry a `targetDefinition` back-reference
naming the Definition they bind to. Component 1.2 documents may instead declare
`targetSurfaceRoutes[]`. Locale 2.0 declares
`target: {kind: "definition" | "app", url, compatibleVersions?}`. App Manifest
does not consume or rewrite those sibling-owned references; it adds a forward
composition graph alongside them.
...
For multi-Definition apps, each form-bound sibling's `targetDefinition`, or
Locale 2.0 definition target, names one Definition. The manifest names which
Definitions are composed but does not retarget sibling references.

A sibling artifact remains independently valid against its own spec without
appearing in an App Manifest. The reverse-discovery path based on
`targetDefinition.url`, or Locale 2.0 `target.kind` plus `target.url`, continues
to work for tools that do not use App Manifest.
...
than once when the loaded Locale documents target different artifacts.

After loading each Locale document, a processor MUST verify:

1. `locales[].locale` equals the loaded document's normalized `locale`;
2. `target.kind: "app"` names this App Manifest's exact `id`, or
   `target.kind: "definition"` names one loaded `definitions[].url`;
3. `target.compatibleVersions`, when present, accepts the loaded target version;
   and
4. no other loaded Locale has the same
   `(target.kind, target.url, normalized locale)` tuple.

An unknown target, reference/document locale mismatch, incompatible target
version, or duplicate tuple is a resolution error. A processor MUST NOT repair
it by retargeting the Locale, choosing the first matching language tag, or
translating Locale 1.0 `targetDefinition` into Locale 2.0 `target`.

Locale fallback remains within the exact target tuple. An app-targeted shell
Locale cannot fall back to a Definition Locale, and one Definition cannot fall
back to another merely because the language tags match.
...
This specification does NOT introduce a `definition.bundle` field,
`Bundle.legacyDefinitionOnly`, a local-id alias for `entrySurface`, an
`entryRoute` override, or a `targetDefinition` alias for Locale 2.0. App
Manifest is a peer primary artifact. Implementations migrate the authored
documents to their new discriminators and fields; processors do not translate
old fields on the 2.4 path.
...
2. Enforce array uniqueness. In v2.4 every `locales[].url` MUST be unique;
   v2.0-v2.3 retain locale-tag uniqueness. Every `mappings[].handle`,
   `components[].handle`, `modules[].id`, `sessions[].id`,
   `definitions[].url`, `dataSources[].url`, and `screeners[].url` MUST be
...
7. Reject unsupported `$formspecBundle` values with
   `unsupported-bundle-version`. A v2.4 processor MUST accept valid older 2.x
   documents under their historical rules.
8. Reject `dataSources[]` before 2.1, `components[]` before 2.2,
   `screeners[]` before 2.3, and `entrySurface` outside 2.4.
9. Apply §3.4.1 exactly: zero Surfaces require absence; one may select
...
| `app-entry-single-implicit-v2-4.json` | positive | One Surface may select implicitly in v2.4 |
| `app-entry-explicit-v2-4.json` | positive | Two Surfaces use exact canonical `entrySurface` |
| `app-locales-target-aware-v2-4.json` | positive | Distinct Locale URLs may repeat a tag for distinct loaded targets |
| `invalid-missing-definition.json` | negative | Schema rejects manifest without `definitions[]` |
| `invalid-duplicate-locale-tag.json` | negative | Historical v2.0-v2.3 semantics reject duplicate `locales[].locale` |
...
| `invalid-entry-surface-missing-v2-4.json` | negative | Schema rejects two Surfaces without `entrySurface` |
| `invalid-entry-surface-without-surfaces-v2-4.json` | negative | Schema rejects `entrySurface` without any Surface |
| `invalid-entry-surface-in-v2-3.json` | negative | Schema rejects the 2.4 member on an older document |
| `invalid-entry-surface-unresolved-v2-4.json` | negative | Cross-artifact validation rejects a local id or non-matching URL without fallback |
| `invalid-duplicate-locale-url-v2-4.json` | negative | v2.4 semantics reject duplicate Locale reference URLs |

A conforming implementation MUST process every fixture in this corpus and produce the documented posture.

The static fixture corpus exercises the single-document parts of §6.1. Exact
entry URL resolution, Locale target identity, sibling-pin consistency, module
coherence, and Screener target resolution require loaded siblings. Their
fixtures state the expected cross-artifact result; they do not substitute a
test-only validator for `AppGraphValidator`.
