...
# Formspec App Manifest Specification v2.1
...
- **App Manifest** -- A JSON document conforming to this specification, identified by `$formspecBundle: "2.0"` or `"2.1"`. The author-facing single composition envelope above a Formspec app. (The wire constant retains the `$formspecBundle` name for symmetry with the v1.0 lineage and to keep validator dispatch trivial; the **document** is the App Manifest.)
...
The `id` MUST be distinct from every sibling URL referenced in the same manifest (every `definitions[].url`, every optional single-cardinality slot's `url`, every entry in `registries[]` / `surfaces[]` / `dataSources[]` / `locales[]` / `mappings[]`). An app's `id` is the app's identity; a sibling's URL is an artifact's identity. Collapsing them would mean the app and one of its parts share an identity, which breaks publication and Trace.
...
The App Manifest spec itself is versioned via `$formspecBundle`. This schema accepts `"2.0"` and `"2.1"`:
...
The `$formspecBundle` value tracks the major.minor of the canonical schema `$id` (`https://formspec.org/schemas/bundleManifest/2.1`). Future minor versions (`2.2`, `2.3`) introduce new sibling slots while remaining backward compatible with older 2.x documents that do not use them, and a major bump (`3.0`) signals another breaking shape change.

A processor that supports only `2.0` MUST check `$formspecBundle` before structural schema validation. A v2.0-only processor MUST reject `"2.1"` documents with `unsupported-bundle-version`; a v2.1 processor MUST accept valid `"2.0"` documents that do not use v2.1-only members. A document with `$formspecBundle` outside the supported 2.x set MUST be rejected with an `unsupported-bundle-version` error rather than a generic schema-validation failure. Documents carrying the legacy `"1.0"` value MUST be rejected by 2.x processors; the 1.0 → 2.0 plural-shape change (ADR 0150 §11.2) is BREAKING and silent acceptance would mis-parse the document.

### 2.3 Sibling Version Pinning

Each sibling reference -- every `definitions[]` entry, every populated optional sibling, every `registries[]`/`surfaces[]`/`dataSources[]`/`locales[]`/`mappings[]` entry -- carries an optional `version` field. When present, `version` MUST be either:
...
The App Manifest schema declares `additionalProperties: false`. New sibling slots require an App Manifest schema version bump. `dataSources[]` is the first such additive slot and requires `$formspecBundle: "2.1"`; a `"2.0"` manifest carrying `dataSources[]` is invalid. The closed surface keeps manifest resolution deterministic.
...
2. Enforce array-uniqueness: every `locales[].locale` MUST be unique within `locales`; every `mappings[].handle` MUST be unique within `mappings`; every `modules[].id` MUST be unique within `modules`; every `sessions[].id` MUST be unique within `sessions`; every `definitions[].url` MUST be unique within `definitions`; every `dataSources[].url` MUST be unique within `dataSources`.
...
7. Reject documents with unsupported `$formspecBundle` values with an `unsupported-bundle-version` error (per §2.2). A v2.1 processor MUST accept valid `"2.0"` documents that do not use v2.1-only members.
8. Reject `dataSources[]` unless `$formspecBundle` is `"2.1"`.
...
- Data Sources loading, source payload fetch/cache behavior, and source-to-slot resolution. App Manifest v2.1 names Data Sources catalogs; `ArtifactResolver` and `AppGraphValidator` own runtime resolution in later ADR 0153 gates.
