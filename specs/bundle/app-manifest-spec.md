...
# Formspec App Manifest Specification v2.2
...
- **App Manifest** -- A JSON document conforming to this specification, identified by `$formspecBundle: "2.0"`, `"2.1"`, or `"2.2"`. The author-facing single composition envelope above a Formspec app. (The wire constant retains the `$formspecBundle` name for symmetry with the v1.0 lineage and to keep validator dispatch trivial; the **document** is the App Manifest.)
...
The `id` MUST be distinct from every sibling URL referenced in the same manifest (every `definitions[].url`, every optional single-cardinality slot's `url`, every entry in `registries[]` / `surfaces[]` / `dataSources[]` / `components[]` / `locales[]` / `mappings[]`). An app's `id` is the app's identity; a sibling's URL is an artifact's identity. Collapsing them would mean the app and one of its parts share an identity, which breaks publication and Trace.
...
The App Manifest spec itself is versioned via `$formspecBundle`. This schema accepts `"2.0"`, `"2.1"`, and `"2.2"`:
...
The `$formspecBundle` value tracks the major.minor of the canonical schema `$id` (`https://formspec.org/schemas/bundleManifest/2.2`). Future minor versions (`2.3`, `2.4`) introduce new sibling slots while remaining backward compatible with older 2.x documents that do not use them, and a major bump (`3.0`) signals another breaking shape change.

A processor that supports only `2.0` MUST check `$formspecBundle` before structural schema validation. A v2.0-only processor MUST reject `"2.1"` and `"2.2"` documents with `unsupported-bundle-version`; a v2.1 processor MUST reject `"2.2"` documents with `unsupported-bundle-version`; a v2.2 processor MUST accept valid `"2.0"` and `"2.1"` documents that do not use unsupported newer members. A document with `$formspecBundle` outside the supported 2.x set MUST be rejected with an `unsupported-bundle-version` error rather than a generic schema-validation failure. Documents carrying the legacy `"1.0"` value MUST be rejected by 2.x processors; the 1.0 → 2.0 plural-shape change (ADR 0150 §11.2) is BREAKING and silent acceptance would mis-parse the document.

### 2.3 Sibling Version Pinning

Each sibling reference -- every `definitions[]` entry, every populated optional sibling, every `registries[]`/`surfaces[]`/`dataSources[]`/`components[]`/`locales[]`/`mappings[]` entry -- carries an optional `version` field. When present, `version` MUST be either:
...
The App Manifest schema declares `additionalProperties: false`. New sibling slots require an App Manifest schema version bump. `dataSources[]` is the first additive slot and requires `$formspecBundle: "2.1"` or later; a `"2.0"` manifest carrying `dataSources[]` is invalid. `components[]` is the second additive slot and requires `$formspecBundle: "2.2"`; `"2.0"` and `"2.1"` manifests carrying `components[]` are invalid. The closed surface keeps manifest resolution deterministic.
...
The opt-in escalation is monotone: an author starts with `definitions: [<one>]`, adds `experience` when task intent becomes worth naming, adds `component` when a single Component tree becomes worth controlling, adds `components[]` when multiple Component documents or route-bound Component identity become worth naming, adds a second Definition when the app composes a second form, adds `surfaces[]` when the app needs presentation shells beyond the form-only path. Each addition is an authored artifact, not a synthesized one.
...
7. Reject documents with unsupported `$formspecBundle` values with an `unsupported-bundle-version` error (per §2.2). A v2.2 processor MUST accept valid `"2.0"` and `"2.1"` documents that do not use unsupported newer members.
8. Reject `dataSources[]` unless `$formspecBundle` is `"2.1"` or `"2.2"`.
9. Reject `components[]` unless `$formspecBundle` is `"2.2"`.
10. Reject a manifest that declares both singular `component` and a `components[]` entry with `handle: "default"`.
...
The static fixture corpus exercises every §6.1 rule that a processor can verify against a single document (rules 1, 2, 3, 5, 7, 8, 9, 10). Rule 4 -- §5.3 sibling-pin consistency -- and rule 6 -- §5.4 module coherence -- require the processor to load the named sibling documents and inspect their discriminators, versions, `targetDefinition` / `targetSurfaceRoutes[]`, and `modules[]` declarations. Those are **online checks**: no static manifest fixture can supply its own siblings. Conformance suites that exercise rules 4 + 6 MUST stub or load the sibling-resolution seam (loaders are §6.3 out of scope for v2).
